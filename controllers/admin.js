...
function createSpreadsheet(id,cb){
  ...
  if(existingSession){
    ...
    teachList.forEach(teach => {
      ...
      overallAnsList.push({name:teach,subjects:teacherSubjectList})
    })
  ...
  if(authentication(req)){
    if(req.query.id == 'all'){
      ...
      allSessions.forEach(session => {
        createSpreadsheet(session.cid, false)
      })
      ...
      createSpreadsheet(session.cid, result => {
        if(result)
